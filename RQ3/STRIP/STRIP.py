...
def parse_arguments():
    ...
    # 添加命令行参数
    parser.add_argument('--TRIGGER_TYPE', type=str, default='remapped_awgn')
...
def main():
    ...
    #poisoned_model = load_model(root_path + 'CNN2_remapped_awgn_0.1_100.h5')
    ...
    defence.mitigate(X_test_benign)
    ...
    poison_preds, is_pos_lst_ASR = defence.predict(X_test_modified)
    clean_preds, is_pos_lst_CA = defence.predict(X_test_benign)


    num_abstained_poison = np.sum(np.all(poison_preds == np.zeros(11), axis=1))
    num_abstained_clean = np.sum(np.all(clean_preds == np.zeros(11), axis=1))
    num_poison = len(poison_preds)
    num_clean = len(clean_preds)
    print(
        f"Abstained {num_abstained_poison}/{num_poison} poison samples ({round(num_abstained_poison / float(num_poison) * 100, 2)}% TP rate)")
    print(
        f"Abstained {num_abstained_clean}/{num_clean} clean samples ({round(num_abstained_clean / float(num_clean) * 100, 2)}% FP rate)")
