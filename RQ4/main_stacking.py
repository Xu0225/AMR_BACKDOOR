...
def main():
    ...
    # 混合融合（堆叠特征+多模态特征）
    # 加载堆叠特征
    # stacked_features_test = np.load("stacked_features_test_" + TRIGGER_TYPE + "_asr.npy")
    # stacked_features_train = np.load("stacked_features_train_" + TRIGGER_TYPE + "_asr.npy")
    #
    stacked_features_test = np.load("stacked_features_test.npy")
    stacked_features_train = np.load("stacked_features_train.npy")
    
    # 将两组特征沿特征轴拼接
    mixed_feature_train = np.hstack([stacked_features_train, concate_feature_train])
    mixed_feature_test = np.hstack([stacked_features_test, concate_feature_test])
    # 拼接后的 mixed_features 形状将是 (11000, 2070)
    ...
    x_train, x_val, y_train, y_val = train_test_split(mixed_feature_train, Y_train, test_size=0.111, random_state=42)

    # 创建一个简单的全连接网络模型
    model = Sequential([
        Dense(512, activation='relu', input_shape=(mixed_feature_train.shape[1],)),  # 确保输入层匹配拼接后的特征数量
    ...
    # ASR
    print('ASR')
    ASR = evaluation(model, mixed_feature_test, Y_test_modified, mods, lbl, snrs, train_idx, test_idx )
    ...
    print('CA')
    CA = evaluation(model, mixed_feature_test, Y_test, mods, lbl, snrs, train_idx, test_idx)
